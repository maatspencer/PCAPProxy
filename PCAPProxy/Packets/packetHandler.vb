...
Public Class packetHandler
    Public Shared Sub Parse(packet As Packet)
        ...
        Using bReader As New BinaryReader(memStream)
            ...
            byteHandler.Endianess(srcPortBytes)
            ...
            byteHandler.Endianess(lenBytes)
            Dim len As UInteger = BitConverter.ToUInt32(lenBytes, 0)

            ' Packet Type
            Dim type As Integer = bReader.ReadByte()

            ' Packet Buffer
            Dim pBuffer As IList(Of Byte) = New List(Of Byte)
            For i = 0 To len - 6
                pBuffer.Add(bReader.ReadByte())
            Next

            ' Decrypt Packet
            RC4.Main(pBuffer, key, False)
